...
PERIODO_PRIMER_DIA=`hive -e "select concat('$PERIODO_ANTERIOR','-01');" | grep -v WARN`
echo "******** PRIMER DIA DEL PERIODO: $PERIODO_PRIMER_DIA ***********"

PERIODO_ULTIMO_DIA=`hive -e "select last_day(concat('$PERIODO_ANTERIOR','-01'));" | grep -v WARN`
echo "******** ULTIMO DIA DEL PERIODOD: $PERIODO_ULTIMO_DIA ***********"

# ========== MF_FORMULAS_SE PROCESS ==========
