...
-- Objective / Objetivo       : Update "MF_Formulas" information on table gb_mdl_mexico_manufactura.MF_Formulas_SE
...
--SET Paso = 18;
INSERT OVERWRITE TABLE gb_mdl_mexico_manufactura.MF_Formulas_se partition(Entidadlegal_id)
SELECT tmp.* from gb_mdl_mexico_manufactura.MF_Formulas_se tmp 
LEFT OUTER JOIN (select Fecha ,EntidadLegal_ID ,MF_Organizacion_ID ,
MF_Producto_ID,SubEnsamble_ID from gb_mdl_mexico_manufactura.MF_Formulas_se where Fecha='${hiveconf:V_PRIMER_DIA}') sec
...
--SET Paso = 19;
INSERT INTO gb_mdl_mexico_manufactura.MF_Formulas_SE PARTITION(entidadlegal_id)
...
FROM gb_mdl_mexico_costoproducir.FORM_ODS_CM_A_01_2  a 
...
(SELECT DISTINCT a.periodo, a.entidadlegal_id, a.planta_id, a.producto_id FROM gb_mdl_mexico_costoproducir.FORM_ODS_CM_A_01_2 a) C
...
LEFT OUTER JOIN gb_smntc_mexico_costoproducir.v_mf_plantas MF_Plantas_0 
...
LEFT OUTER JOIN ( SELECT c.Periodo, c.EntidadLegal_ID, c.MF_Organizacion_ID, c.planta_id, c.MF_Producto_ID, SUM(c.Costo) Costo
  FROM gb_mdl_mexico_manufactura.MF_Costo_Prod c
...
FROM gb_mdl_mexico_costoproducir_views.V_ENTIDADESLEGALES_ACTIVAS_FOR c
 WHERE TRIM(c.Aplicacion) = 'FORMULAS' GROUP BY c.EntidadLegal_ID);

-- compactacion
insert overwrite table gb_mdl_mexico_manufactura.mf_formulas_se partition(entidadlegal_id) select tmp.* from gb_mdl_mexico_manufactura.mf_formulas_se tmp join (select fecha, entidadlegal_id, mf_organizacion_id, mf_producto_id, subensamble_id, max(storeday) as first_record from gb_mdl_mexico_manufactura.mf_formulas_se group by fecha, entidadlegal_id, mf_organizacion_id, mf_producto_id, subensamble_id) sec on tmp.fecha = sec.fecha and tmp.entidadlegal_id = sec.entidadlegal_id and tmp.mf_organizacion_id = sec.mf_organizacion_id and tmp.mf_producto_id = sec.mf_producto_id and tmp.subensamble_id = sec.subensamble_id and tmp.storeday = sec.first_record;
insert overwrite table gb_mdl_mexico_manufactura.mf_formulas_se partition(entidadlegal_id) select distinct * from gb_mdl_mexico_manufactura.mf_formulas_se;
