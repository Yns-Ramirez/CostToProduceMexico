...
insert into cp_app_costoproducir.CP_Lineas_Prod_Metros partition(entidadlegal_id) 
...
from cp_view.VT_CP_Lineas_Prod_Metros b) b
...
insert overwrite table cp_app_costoproducir.CP_Lineas_Prod_Metros partition(entidadlegal_id) 
select tmp.* from cp_app_costoproducir.CP_Lineas_Prod_Metros tmp join 
(select  entidadlegal_id, MF_Organizacion_ID, Planta_ID, Linea_Prod_ID, Fecha_Ini, max(storeday) as first_record 
from cp_app_costoproducir.CP_Lineas_Prod_Metros 
...
FROM_UNIXTIME(UNIX_TIMESTAMP()),a.entidadlegal_id from cp_view.VT_CP_Lineas_Prod_Metros a;
