...
insert into table gb_mdl_mexico_costoproducir.wrkt_a_rubros_fsg partition(entidadlegal_id)
     select fsg.* from gb_smntc_mexico_costoproducir.v_a_rubros_fsg fsg
          where fsg.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id) and fsg.aniosaldo = substr('${hiveconf:speriod}',1, 4)
          and fsg.messaldo  = substr('${hiveconf:speriod}',6, 2);

-- compactation
insert overwrite table gb_mdl_mexico_costoproducir.wrkt_a_rubros_fsg partition(entidadlegal_id) select tmp.* from gb_mdl_mexico_costoproducir.wrkt_a_rubros_fsg tmp join (select aniosaldo, messaldo, entidadlegal_id, linea_id, areanegocio_id, cuentanatural_id, centrocostos_id, analisislocal_id, max(storeday) as first_record from gb_mdl_mexico_costoproducir.wrkt_a_rubros_fsg group by aniosaldo, messaldo, entidadlegal_id, linea_id, areanegocio_id, cuentanatural_id, centrocostos_id, analisislocal_id) sec on tmp.aniosaldo = sec.aniosaldo and tmp.messaldo = sec.messaldo and  tmp.entidadlegal_id = sec.entidadlegal_id and tmp.linea_id = sec.linea_id and tmp.areanegocio_id = sec.areanegocio_id and tmp.cuentanatural_id = sec.cuentanatural_id and tmp.centrocostos_id = sec.centrocostos_id and  tmp.analisislocal_id = sec.analisislocal_id and tmp.storeday = sec.first_record;

-- ======================================================
-- cp_operarios

-- step 1 
insert overwrite table gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_p_1
select
     ep.empleado_id
     ,p.posicion_id
     ,p.puesto_id
     ...
     ,ep.fechafinposicion
from gb_smntc_mexico_costoproducir.e_empleado_posicion ep
     join gb_smntc_mexico_costoproducir.e_posicion p on ep.posicion_id = p.posicion_id 
     join 
          (
          ...
          select
               ...
               ,fsg.nombreconcepto
               ,fsg.centrocostos_id
          from gb_mdl_mexico_costoproducir.wrkt_a_rubros_fsg fsg
               join gb_smntc_mexico_costoproducir.v_mf_plantas p on fsg.entidadlegal_id = p.entidadlegal_id and fsg.areanegocio_id = p.planta_id 
               join (
                         ...
                         from gb_smntc_mexico_costoproducir.v_mf_lineas_prod_centro_costos lc
                              inner join gb_smntc_mexico_costoproducir.v_mf_plantas p on lc.entidadlegal_id = p.entidadlegal_id and lc.mf_organizacion_id = p.mf_organizacion_id and lc.planta_id = p.planta_id
                         where lc.dl = 1 and lc.fecha_fin is null  
                         and lc.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id)
               ...
               join (
                    select  cpj.entidadlegal_id , trim(cpj.condicion) as condicion from gb_smntc_mexico_costoproducir.cp_parametros cpj where cpj.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id) and lower(cpj.objeto) = lower('cp_operarios') and lower(cpj.campo) = lower('cuentanatural_id') and cpj.subrubro_id is null group by cpj.entidadlegal_id,trim(cpj.condicion)
                    ) cpj2 
                    on fsg.entidadlegal_id = cpj2.entidadlegal_id and fsg.cuentanatural_id = cpj2.condicion
          where fsg.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id)
...
where p.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id)
...
-- step 2
insert overwrite table gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_e_2
select
...
--step 3
insert overwrite table gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_m_3
select
e.periodo
,e.entidadlegal_id
...
-- step 2
from gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_e_2 e
inner join
(
     select
          m2.periodo
          ...
          (
               ...
               from gb_smntc_mexico_costoproducir.v_mf_lineas_prod_centro_costos lc
                    ...
                    (
                         ...
                         from gb_smntc_mexico_costoproducir.cp_medidas_prorrateo cmpi
                         where cmpi.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id)  and tipomedida_id = 1 and periodo = '${hiveconf:speriod}'
               ...
               where lc.entidadlegal_id in (select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id) and lc.dl = 1 and lc.fecha_fin is null 
...
-- step 4
insert overwrite table gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_t_4
select
...
from gb_mdl_mexico_costoproducir_views.v_mf_produccion pr
     inner join 
          (
               ...
               from gb_mdl_mexico_costoproducir_views.v_mf_produccion pr2
                    join 
                         (
                              select
                                   j.entidadlegal_id
                                   ,count (distinct j.turno_id) as total_turnos_default
                              from gb_mdl_mexico_manufactura.mf_turno_default j
               ...
               where pr2.linea_prod_id <> -1 and pr2.total_registrado <> 0
                    ...
                    and pr2.entidadlegal_id in (
                         select entidadlegal_id from gb_mdl_mexico_costoproducir_views.v_entidadlegal_activas group by entidadlegal_id
                         )
...
-- step 5
insert into gb_smntc_mexico_costoproducir.cp_operarios partition(entidadlegal_id)
select
     m.periodo
     ...
     ,m.entidadlegal_id
      -- step 3
from gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_m_3 m
     -- step 4
     inner join gb_mdl_mexico_costoproducir_wrkt.wkrt_cp_operarios_t_4 t
