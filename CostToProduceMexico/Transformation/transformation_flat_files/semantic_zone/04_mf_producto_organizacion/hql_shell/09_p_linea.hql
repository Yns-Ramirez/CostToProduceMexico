...
-- Objective / Objetivo       : Update information weekly, table cp_dwh.P_LINEA
-- Subject Area / Area Sujeto : Product

-- Insert rows from flat file product
INSERT INTO cp_dwh.P_LINEA
...
-- Delete duplicades  
insert overwrite table cp_dwh.P_LINEA
select tmp.* from cp_dwh.P_LINEA tmp
join (select Categoria_ID,Linea_ID,max(storeday) as first_record 
from cp_dwh.P_LINEA group by Categoria_ID,Linea_ID) sec
