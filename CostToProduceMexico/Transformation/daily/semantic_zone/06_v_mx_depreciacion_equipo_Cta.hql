-- ======================================================
--  gb_mdl_mexico_costoproducir.mx_depreciacion_equipo_cuenta
truncate table gb_mdl_mexico_costoproducir.mx_depreciacion_equipo_cuenta;
insert into gb_mdl_mexico_costoproducir.mx_depreciacion_equipo_cuenta partition(storeday)
select 
...
from
 ...
 left outer join gb_mdl_mexico_costoproducir_views.gl_code_combinations_fa_brio_tmp cc
...
-- compactation
insert overwrite table gb_mdl_mexico_costoproducir.mx_depreciacion_equipo_cuenta partition(storeday) select tmp.* from  gb_mdl_mexico_costoproducir.mx_depreciacion_equipo_cuenta tmp join (select activoequipoerp_id ,fecha, max(storeday) as first_record from  gb_mdl_mexico_costoproducir.mx_depreciacion_equipo_cuenta group by activoequipoerp_id ,fecha) sec on tmp.activoequipoerp_id = sec.activoequipoerp_id and tmp.fecha = sec.fecha and tmp.storeday = sec.first_record;
